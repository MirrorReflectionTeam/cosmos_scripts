...
sed -i.bak -e "s%^proxy_app = \"tcp://127.0.0.1:26658\"%proxy_app = \"tcp://127.0.0.1:${array_rpc[$PORT]}658\"%; s%^laddr = \"tcp://127.0.0.1:26657\"%laddr = \"tcp://127.0.0.1:${array_rpc[$PORT]}657\"%; s%^pprof_laddr = \"localhost:6060\"%pprof_laddr = \"localhost:${array_laddr[$PORT]}60\"%; s%^laddr = \"tcp://0.0.0.0:26656\"%laddr = \"tcp://0.0.0.0:${array_rpc[$PORT]}656\"%; s%^prometheus_listen_addr = \":26660\"%prometheus_listen_addr = \":${array_rpc[$PORT]}660\"%" $HOME/.$1/config/config.toml && sed -i.bak -e "s%^address = \"0.0.0.0:9090\"%address = \"0.0.0.0:${array_gRPC_web[$PORT]}90\"%; s%^address = \"0.0.0.0:9091\"%address = \"0.0.0.0:${array_gRPC_web[$PORT]}91\"%; s%^address = \"tcp://0.0.0.0:1317\"%address = \"tcp://0.0.0.0:${array_api[$PORT]}17\"%" $HOME/.$1/config/app.toml && sed -i.bak -e "s%^node = \"tcp://localhost:26657\"%node = \"tcp://localhost:${array_rpc[$PORT]}657\"%" $HOME/.$1/config/client.toml 